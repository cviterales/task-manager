...
const closeTaskReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case actionTypes.SET_TASK_RECOVERED_EQUIPMENT:
      let currentRecoveredEquipment = state.equipment_recovered
    ...
    case actionTypes.SET_TASK_UPDATED_EQUIPMENT:
      let currentUpdatedEquipments = state.equipment_updated
      let index = state.equipment_updated.findIndex(
        (el) => el.id_sub_cta_equipment === action.payload.id_sub_cta_equipment
      )
      if (index >= 0) {
    ...
    case actionTypes.SET_TASK_TECHNICAL_DATA:
      ...
      newTechnicalData[action.payload.target.name] = action.payload.target.value
      return {
        ...state,
    ...
    case actionTypes.UPDATE_TASK_MATERIALS:
      let newMaterials = state.materials.filter((item) => item !== action.payload)
      return {
        ...state,
